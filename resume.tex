...
\definecolor{mycolor}{HTML}{F0F0EB}

% Set as default text color
\color{mycolor}
%-----------------------------------------
...
\definecolor{accent}{HTML}{CC785C}
\definecolor{accent_text}{HTML}{D4A27F}

\definecolor{rule}{HTML}{BFBFBA}

\newcommand{\EM}[1]{\textbf{\textcolor{accent_text}{#1}}}

\titleformat{\section}
{\large\bfseries}{}
{0em}{\textcolor{accent}}[\color{rule}\noindent\rule{\dimexpr\textwidth-5cm\relax}{0.4pt}\vspace{0pt}]
%-------------------------------------
...
%__________________BG_______________________
\usepackage{eso-pic}
\usepackage{xcolor}
\usepackage{geometry}
\usepackage{iftex}
...
% ── Backgrounds ──────────────────────────────────────────────
% Front page background (only first page)
% \AddToShipoutPictureBG*{%
%   \put(0,0){%
%     \pdfximage width \paperwidth height \paperheight {Front.pdf}%
%     \pdfrefximage\pdflastximage
%   }%
% }

% Rest pages background (all pages after first)
...
\AddToShipoutPictureBG*{%
  \put(0,0){%
    \ifpdftex
      ...
      \pdfrefximage\pdflastximage
    \else
      \includegraphics[width=\paperwidth, height=\paperheight]{Front.pdf}%
    \fi
  }%
}
\newcounter{bgpage}
...
\AddToShipoutPictureBG{%
  \ifnum\value{bgpage}>0
    \put(0,0){%
      \ifpdftex
        \pdfximage page 1 width \paperwidth height \paperheight {Rest.pdf}%
        \pdfrefximage\pdflastximage
      \else
        \includegraphics[width=\paperwidth, height=\paperheight, page=1]{Rest.pdf}%
      \fi
    }%
  \fi
  \stepcounter{bgpage}%
...
\section{Data analyst}
  ...
  \small{
  PhD student in urban and real estate economics who uses econometric and machine learning methods to solve problems with \EM{spatial data}. I have experience managing the entire data lifecycle, from designing data infrastructure and \EM{SQL-based} workflows to building dashboards and sharing results with both \EM{technical} and \EM{non-technical} audiences. Good at \EM{Python}, \EM{R}, and \EM{SQL}, and has a strong sense of how to turn vague stakeholder needs into clear, actionable data work and \EM{visualisations} that make complicated patterns easy to understand and ready to make decisions.
  }
...
\begin{tabular*}{\textwidth}{r@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}l}
  \textbf{Python:}              &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE  $ }& &
  \textbf{PostgreSQL:}          &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE  $ }& &
  \textbf{DuckDB:}              &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ }& &
  \textbf{SQL:}                 &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE  $ }& &
  \\  

  \textbf{Spatial data:}        &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE \CIRCLE$ }& &
  \textbf{LLM/NLP/LLM:}         &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ }& &
  \textbf{SQLite:}              &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE  $ }& &
  \textbf{AZURE:}               &   \textcolor{accent_text}{$\CIRCLE \CIRCLE   $ }& &
  \\  

  \textbf{GCP:}                 &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ }& &
  \textbf{R:}                   &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ } & &
  \textbf{MongoDB:}             &   \textcolor{accent_text}{$\CIRCLE \CIRCLE   $ }& &
  \textbf{Excel:}               &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ }& &
  \\  

  \textbf{AcrGIS:}              &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE $ }& &
  \textbf{ETL/ELT:}             &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ }& &
  \textbf{Cartography:}         &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ }& &
  \textbf{STATA:}               &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $ }& &
  \\  

  \textbf{Git:}                 &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE $ }& &
  \textbf{RAG:}                 &   \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE \CIRCLE$ }& &
  \textbf{Julia:}               &   \textcolor{accent_text}{$\CIRCLE \CIRCLE   $ }& &
  \textbf{PowerBI:}             &   \textcolor{accent_text}{$\CIRCLE \CIRCLE $ }& &
  \\
...
\begin{tabular*}{\textwidth}{r@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}l}
  \textbf{Stakeholde management:}&  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Visual communication:} &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Task management:}      &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \\  
  \textbf{Team work:}            &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE  $}& &
  \textbf{Self-organised:}       &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE  $}& &
  \textbf{Project management:}   &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $}
  \\  
  \textbf{Accounting:}           &  \textcolor{accent_text}{$\CIRCLE \CIRCLE   $}& &
  \textbf{Quality control:}      &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Graphic Design:}       &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE \CIRCLE$} 
  \\  
  \textbf{Quantiative research:} &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Prompt engineering:}   &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Document Management:}  &  \textcolor{accent_text}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \\
...
Stakeholders asked for spatial data on all buildings in Finland that could be used in other EU countries. The main problem was figuring out what data was needed and how to get it and put it all together. After two meetings with stakeholders to get everyone on the same page, we came up with a two-phase plan. The first phase is to map out the whole data model and build the data flow into the application. The second phase is to implement the model through several iterations, gradually refactoring the architecture to move data operations outside of the core system, which will make it easier to scale and separate concerns.

\resumeItemListStart{}
\resumeItem{Developing a data pipeline to transform and compute key metrics from building spatial data across Finland.}
\resumeItem{Designing and implementing an \EM{API-based pipeline} for automated data operations in Finland.}
...
\resumeItemListStart{}
\resumeItem{Identified and resolved \EM{technical debt}, significantly improving data pipeline performance and maintainability.}
\resumeItem{Led migration from \EM{Pandas} to \EM{Polars} (including GeoPandas)}

\resumeItem{Refactored codebase design patterns and produced \EM{technical documentation}.}
\resumeItem{Expanded and improved \EM{test coverage} and testing methodologies for data processing workflows.}
\resumeItemListEnd{}
...
\resumeItemListStart{}
\resumeItem{Preparing and maintaining \EM{data visualizations} for academic presentations and lectures.}
\resumeItem{Designing data-pipelines for local AI to extract features from text.}
% \resumeItem{Providing IT service and assitance to my research team to resolve issues with their computers and software.}
\resumeItem{Applying Econometric, ML, time-series and data analysis.}
% \resumeItem{Processing HTML and PDF files and implementing \EM{RAG} to extract structured data from the land use plan documents.}
% \resumeItem{Conducting \EM{econometric} and \EM{hedonic regression} analyses on property markets, housing development, and real estate dynamics.}
\resumeItem{Acquiring GIS data from REST and ArcGIS Online endpoints.}
\resumeItem{Deploying and maintaining \EM{Azure Functions} to automate web scraping of property listings and store real estate data for research purposes.}
\resumeItem{Performing \EM{GIS analysis} to produce and maintain geospatial research datasets and geocode real estate transactions from data sources such as MML, Ryhti, SYKE, and KVKL.}
\resumeItem{Designing and deploying research databases using \EM{PostgreSQL}, \EM{DuckDB}, and data \EM{hive partitions}}
...
\resumeItem{Data stewardship responsibilities to acquire, curate, and distribute research datasets.}
...
\begin{tabular*}{\textwidth}{r@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}lcr@{\extracolsep{\fill}}l}
  \textbf{Uni-variate time-series:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE$}& &
  \textbf{VAR time-series:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE  $}& &
  \textbf{Co-integration time-series:}& \textcolor{accent}{$\CIRCLE \CIRCLE   $} & &
  \\
  \textbf{multi-variate time-series:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE  $}& &
  \textbf{OLS:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE \CIRCLE$} & &
  \textbf{IV-OLS:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \\
  \textbf{Panel data:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Generalised Method of Moments:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Generalised linear models:}& \textcolor{accent}{$\CIRCLE \CIRCLE $} & &
  \\
  \textbf{Spatial econometrics:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE $} & &
  \textbf{Survival/time-to-event:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE \CIRCLE$} & &
  \textbf{Additive hazard models:}& \textcolor{accent}{$\CIRCLE \CIRCLE \CIRCLE \CIRCLE \CIRCLE$} & &
  \\
